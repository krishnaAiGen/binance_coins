...
class SlackNotifier:
        ...
        try:
            ...
            if trade_info.get('success', False):
                ...
                message = {
                    ...
                    "Position": "LONG",
                    "Status": "Position opened with SL and TP set"
                }
            else:
                # Failed trade notification
                message = {
                    "🚨 NEW LISTING TRADE FAILED": "❌ ERROR",
                    "Time": timestamp,
                    "Symbol": trade_info.get('symbol', 'N/A'),
                    "Error": trade_info.get('error', 'Unknown error'),
                    "Action": "Will retry in 3 seconds"
                }
    ...
    def post_new_listing_alert(self, symbol):
        """Post a new listing detection alert to Slack"""
        if not self.webhook_url:
            print("Slack webhook URL not configured")
            return False
        ...
        try:
            ...
            message = {
                "🚀 NEW LISTING DETECTED": "🔥 TRADING OPPORTUNITY",
                "Time": timestamp,
                "Symbol": symbol,
                "Status": "Attempting to place long position...",
                "Action": "Bot is executing trade strategy"
            }
            
            return self.post_to_slack(message)
            
        except Exception as e:
            print(f"Error creating new listing alert: {e}")
            return False
        ...
        try:
            ...
            if success:
                ...
                message = {
                    ...
                    "Symbol": symbol,
                    "Attempt": f"#{attempt_number}",
                    "Status": "Will retry again in 3 seconds"
                }
