...
class OneHourDataDownloader:
    def __init__(self):
        self.base_url = "https://fapi.binance.com/fapi/v1/klines"
        self.data_folder = "/Users/krishnayadav/Documents/test_projects/binance_coins/utils/short_1hr/data"
        self.launch_dates = {}
    ...
    def download_7d_hours_from_launch(self, symbol, launch_date_str):
        """
        Download 7 days of 1-hour candles (168 candles) from launch date
        Single API call since 168 < 1000 limit
        """
        try:
            # Parse launch date
            ...
            target_candles = 168  # 7 days * 24 hours
            
            print(f"Downloading {target_candles} 1h candles for {symbol}...", end=" ")
            
            # Single API call for 168 hours (within Binance's 1000 limit)
            params = {
                'symbol': symbol,
                'interval': '1h',
    ...
    def save_data(self, symbol, df, launch_date_str):
        ...
        filename = f"{self.data_folder}/{symbol}_1h_7d_{launch_date_clean}.csv"
    ...
    def filter_valid_launch_dates(self):
        ...
        for symbol, launch_date_str in self.launch_dates.items():
            try:
                ...
                # Check if launch date is in the past (at least 7 days ago for safety to get full 7d data)
                if launch_date < current_time - timedelta(days=7):
        ...
        print(f"   ✅ Valid dates (at least 7d old): {len(valid_dates)}")
        print(f"   ⏳ Too recent/future dates (skipped): {len(future_dates)}")
        print(f"   ❌ Invalid dates (skipped): {len(invalid_dates)}")
        
        if future_dates:
            print(f"\n⏳ Skipping too recent dates (first 5): {[s for s, d in future_dates[:5]]}")
        if invalid_dates:
            print(f"\n❌ Skipping invalid dates: {[s for s, d in invalid_dates]}")
        
        return valid_dates

    def download_all_1h_launch_data(self, delay=0.3):
        """Download 7 days of 1-hour data from launch for all coins"""
        print(f"🚀 Starting download of 7-day 1-hour launch data...")
        print(f"⚠️  Each coin will have 168 candles (7 days × 24 hours)")
        print(f"📦 Single API call per coin (168 < 1000 limit)")
        ...
        try:
            for i, (symbol, launch_date_str) in enumerate(valid_dates.items(), 1):
                ...
                try:
                    ...
                    # Download 7d 1-hour data from launch
                    klines_data = self.download_7d_hours_from_launch(symbol, launch_date_str)
        ...
        print(f"📊 Total 1-hour candles downloaded: {len(successful_downloads) * 168:,}")
    ...
    def analyze_1h_launch_data(self):
        """Analyze the downloaded 1-hour launch data"""
        print("\n🔍 ANALYZING 1-HOUR 7-DAY LAUNCH DATA...")
        print("=" * 80)
        
        csv_files = [f for f in os.listdir(self.data_folder) if f.endswith('.csv') and '_1h_7d_' in f]
        
        if not csv_files:
            print("No 1-hour data files found to analyze")
            return
        ...
        for filename in csv_files:
            try:
                ...
                if len(df) > 0:
                    ...
                    # Calculate performance for 7-day period (using 1-hour data)
                    first_price = df['open'].iloc[0]
                    last_price = df['close'].iloc[-1]
                    high_7d = df['high'].max()
                    low_7d = df['low'].min()
                    
                    performance_7d = ((last_price - first_price) / first_price) * 100
                    max_gain_7d = ((high_7d - first_price) / first_price) * 100
                    max_loss_7d = ((low_7d - first_price) / first_price) * 100
                    
                    total_volume = df['volume'].sum()
                    
                    # Calculate 1-hour, 4-hour, 24-hour, 3-day performance
                    performance_1h = ((df['close'].iloc[0] - first_price) / first_price) * 100 if len(df) > 0 else 0
                    performance_4h = ((df['close'].iloc[3] - first_price) / first_price) * 100 if len(df) > 3 else 0
                    performance_24h = ((df['close'].iloc[23] - first_price) / first_price) * 100 if len(df) > 23 else 0
                    performance_3d = ((df['close'].iloc[71] - first_price) / first_price) * 100 if len(df) > 71 else 0
                    
                    analysis_data.append({
                        'symbol': symbol,
                        'launch_date': launch_date,
                        'first_price': first_price,
                        'last_price_7d': last_price,
        ...
        if analysis_data:
            ...
            print(f"📈 1-HOUR 7-DAY LAUNCH DATA ANALYSIS:")
            print(f"   Total coins analyzed: {len(analysis_df)}")
            print(f"   Average 7d performance: {analysis_df['performance_7d'].mean():.2f}%")
            print(f"   Average 1h performance: {analysis_df['performance_1h'].mean():.2f}%")
            ...
            print(f"   Average 3d performance: {analysis_df['performance_3d'].mean():.2f}%")
            print(f"   Best 7d performance: {analysis_df['performance_7d'].max():.2f}% ({analysis_df.loc[analysis_df['performance_7d'].idxmax(), 'symbol']})")
            print(f"   Worst 7d performance: {analysis_df['performance_7d'].min():.2f}% ({analysis_df.loc[analysis_df['performance_7d'].idxmin(), 'symbol']})")
            
            print(f"\n🏆 TOP 10 BEST 7D PERFORMERS (1-hour data):")
            top_performers = analysis_df.nlargest(10, 'performance_7d')[['symbol', 'performance_1h', 'performance_4h', 'performance_24h', 'performance_3d', 'performance_7d', 'launch_date']]
            print(top_performers.to_string(index=False))
            
            print(f"\n📉 TOP 10 WORST 7D PERFORMERS (1-hour data):")
            worst_performers = analysis_df.nsmallest(10, 'performance_7d')[['symbol', 'performance_1h', 'performance_4h', 'performance_24h', 'performance_3d', 'performance_7d', 'launch_date']]
            print(worst_performers.to_string(index=False))
            
            # Save analysis
            analysis_filename = f"{self.data_folder}/1h_7d_launch_analysis.csv"
    ...
    def show_sample_data(self, num_samples=3):
        ...
        csv_files = [f for f in os.listdir(self.data_folder) if f.endswith('.csv') and 'analysis' not in f and '_1h_7d_' in f]
        
        if not csv_files:
            print("No 1-hour sample data to show")
            return
        
        print(f"\n📋 SAMPLE 1-HOUR DATA (First {num_samples} coins):")
        print("=" * 80)
        ...
        for filename in csv_files[:num_samples]:
            try:
                ...
                print(f"First 24 hours of trading (1-hour intervals):")
                sample_df = df[['timestamp', 'open', 'high', 'low', 'close', 'volume', 'hours_from_launch']].head(24)
                ...
                print(f"   7d performance: {((df['close'].iloc[-1] - df['open'].iloc[0]) / df['open'].iloc[0]) * 100:.2f}%")
                
            except Exception as e:
                print(f"Error showing sample for {filename}: {e}")

def main():
    print("🚀 === 1-Hour 7-Day Launch Data Downloader ===")
    print("Downloads 7 days of 1-hour candlestick data from each coin's launch date")
    print("⚠️  Each coin will have 168 candlesticks (7 days × 24 hours)")
    print("=" * 80)
    
    downloader = OneHourDataDownloader()
    ...
    print(f"📦 Single API call per coin (168 < 1000 limit) - No batching needed!")
    ...
    # Download all 1-hour 7-day launch data
    successful, failed = downloader.download_all_1h_launch_data(delay=0.3)
    
    if successful:
        # Show sample data
        downloader.show_sample_data(2)
        
        # Analyze 1-hour launch performance
        analysis_df = downloader.analyze_1h_launch_data()
        
        print(f"\n✅ 1-hour 7-day launch data download complete!")
        print(f"📁 Data saved in: {os.path.abspath(downloader.data_folder)}/")
        print(f"🎯 Ready for trading analysis with {len(successful)} coins")
        print(f"📊 Total 1-hour candles downloaded: {len(successful) * 168:,}")
    else:
